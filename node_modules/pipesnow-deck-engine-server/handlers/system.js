...
const detailedHealthCheck = asyncHandler(async (req, res) => {
  try {
    ...
    // Try to get global status with fallback
    let globalStatus;
    try {
      globalStatus = engine.getGlobalStatus();
    } catch (statusError) {
      console.warn("[WARN] getGlobalStatus failed:", statusError.message);
      globalStatus = {
        decks: { count: 0, names: [] },
    ...
    console.error("[ERROR] detailedHealthCheck failed:", error.message);
    healthResponse(res, {
      status: "unhealthy",
      version: "unknown",
      uptime: process.uptime(),
